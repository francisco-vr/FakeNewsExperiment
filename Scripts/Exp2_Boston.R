...
Model6 <-glm(SC0 ~ E2Treat_Afin + E2Treat_Opuesto + HomoIndex + DigitIndex + Age_2 + Age_3 + Age_4 + Educ_2
             ...
             data = df)
Model7 <-glm(SC0 ~ E2Treat_Afin + E2Treat_Opuesto + HomoIndex + DigitIndex + Age_2 + Age_3 + Age_4 + Educ_2
             + Educ_3 + Educ_4 + Educ_5 + ideologia_Izquierda + ideologia_Derecha + ideologia_Ninguno
             + NivEco_2 + NivEco_3 + NivEco_4 + NivEco_5,
             data = df)

Reg1Bal <-modelsummary::msummary(list(Model1, Model2, Model3), statistic = 'p.value', starts = TRUE)
Reg1Bal
# Regresion with unbalanced covariates


Reg1NoBa <-modelsummary::msummary(list(Model1, Model2, Model3, Model4, Model5, Model6,Model7))
Reg1NoBa
